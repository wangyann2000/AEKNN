    ...
    def __init__(self, model_config):
        ...
        if model_config['diversity'] == False:
            if model_config['plearn'] == False:
                self.basis_vector = nn.Parameter(torch.rand(model_config['basis_vector_num'], self.hidden_dim),
                                                 requires_grad=False)
            else:
                self.basis_vector = nn.Parameter(torch.rand(model_config['basis_vector_num'], self.hidden_dim),
                                                 requires_grad=True)
            ...
            if model_config['plearn'] == False:
                self.basis_vector = nn.Parameter(
                    torch.tensor(random_orthogonal_vectors(model_config['basis_vector_num'], self.hidden_dim)).float(),
                    requires_grad=False)
            else:
                self.basis_vector = nn.Parameter(
                    torch.tensor(random_orthogonal_vectors(model_config['basis_vector_num'], self.hidden_dim)).float(),
